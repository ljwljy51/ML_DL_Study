...
parser.add_argument(
    "--opts", nargs="+", default=["random", 30.0], dest="opts"
...
if mode == "train":  # train mode인 경우
    ...
    transform_val = transforms.Compose(
        [RandomCrop(shape=(ny, nx)), Normalization(mean=0.5, std=0.5), ToTensor()]
    )
...
if network == "unet":
    ...
    net = Hourglass(nch=nch, nker=nker, norm="bnorm", learning_type=learning_type).to(
        device
    )
# elif network=="resnet":
...
if mode == "train":
    ...
    for epoch in range(st_epoch + 1, num_epoch + 1):
        ...
        # 모델 저장
        if epoch % 20 == 0:  # epoch 특정 횟수마다 저장하고싶으면
    ...
    with torch.no_grad():
        ...
        for batch, data in enumerate(loader_test, 1):
            ...
            for j in range(label.shape[0]):
                ...
                # numpy 저장
                np.save(
                    os.path.join(result_dir, "numpy", "%04d_label.npy" % id),
                    label_,
                )
                np.save(
                    os.path.join(result_dir, "numpy", "%04d_input.npy" % id),
                    input_,
                )
                np.save(
                    os.path.join(result_dir, "numpy", "%04d_output.npy" % id),
                    output_,
                )
                ...
                # png 저장
                plt.imsave(
                    os.path.join(result_dir, "png", "%04d_label.png" % id),
                    label_,
                    cmap="gray",
                )
                plt.imsave(
                    os.path.join(result_dir, "png", "%04d_input.png" % id),
                    input_,
                    cmap="gray",
                )
                plt.imsave(
                    os.path.join(result_dir, "png", "%04d_output.png" % id),
                    output_,
                    cmap="gray",
                )
